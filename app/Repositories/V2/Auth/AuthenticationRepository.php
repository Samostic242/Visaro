 ...
 class AuthenticationRepository implements AuthenticationRepositoryInterface
 {
    public function login(array $data)
    {
        if(auth()->attempt($data))
        {
            if(!auth()->user()->active)
            {
                return \respondError(400, 'Your account is not active kindly reach out to our customer support to re-activate');
            }elseif(auth()->user()->email_verified_at == NULL)
            {
                return \respondError(400, 'Your account has not been verified, kindly proceed to verify your account');
            }elseif(auth()->user()->phone_verified_at !== NULL)
            {
                return \respondError(400, 'Your phone number has not been verified, kindly proceed to verify your account');
            }
        $token = auth()->user()->createToken('token')->accessToken;
        $userdata = ['token' => $token];
        return \respondSuccess('User Logged in successfully', $userdata);
        }
        return \respondError(401, 'Incorrect username or password');
    ...
    public function forgotPassword(array $data)
    {
        ...
        {
            return \respondError(400, 'An error occurred, please try again');
        }
